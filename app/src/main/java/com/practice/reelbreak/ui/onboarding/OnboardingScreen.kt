...
fun OnboardingScreen(
    ...
    // Use configuration to make layout responsive
    val screenHeight   = config.screenHeightDp.dp
    val isCompact      = screenHeight < 700.dp   // small phones (< ~700 dp height)
    ...
    Box(
        modifier = Modifier
            .fillMaxSize()
            .background(
                // Match app dark background
                Brush.verticalGradient(
                    listOf(Color(0xFF0D0618), Color(0xFF130A22), Color(0xFF0D0618))
                )
            )
    ) {
        // Decorative blurred orbs in the background
        Canvas(modifier = Modifier.fillMaxSize()) {
            drawOrbBackground(orbOffset)
        }
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            HorizontalPager(
                ...
                when (pages[index]) {
                    ...
                    OnboardingPageType.HOW_IT_WORKS -> HowItWorksPage(isCompact = isCompact)
                    OnboardingPageType.LIMITS_SETUP -> LimitsSetupPage(isCompact = isCompact)
                    ...
                    OnboardingPageType.ALL_SET      -> AllSetPage(isCompact = isCompact)
                }
            }
...
private fun WelcomePage(isCompact: Boolean) {
    PageLayout(isCompact = isCompact) {
        ...
        ImagePlaceholder(
            ...
            // Replace R.drawable.onboard_welcome when you have your image:
...
// ─────────────────────────────────────────────────────────────────────────────
// PAGE 3 — LIMITS SETUP  (informational — actual limits set in Dashboard)
// ─────────────────────────────────────────────────────────────────────────────
@Composable
...
private fun PermissionsPage(
    ...
    PageLayout(isCompact = isCompact) {
        // ── Small decorative icon instead of large image on this page ─
        Box(
            modifier = Modifier
...
private fun AllSetPage(isCompact: Boolean) {
    PageLayout(isCompact = isCompact) {
        ...
        Text(
            text      = "ReelBreak is ready to guard your focus.\nScroll less. Live more.",
            color     = Color.White.copy(alpha = 0.58f),
            fontSize  = if (isCompact) 13.sp else 15.sp,
            textAlign = TextAlign.Center,
            lineHeight = 22.sp,
 ...
 * - Fills width, scrollable vertically to handle overflow on small screens.
 * - Horizontally centered content, consistent padding.
 */
@Composable
 ...
 * Replace `imageRes` comment with your actual drawable once you have the images.
 * Maintains a consistent aspect ratio so layout doesn't shift.
 */
@Composable
private fun ImagePlaceholder(
    ...
    val imageHeight = if (isCompact) 180.dp else 230.dp
